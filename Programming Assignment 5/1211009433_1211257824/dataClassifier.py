...
def enhancedPacmanFeatures(state, action):
    ...
    features['foodOnBoard'] = state.getNumFood()
    foodLocation = state.getFood().asList()
    minFoodDist = 0
    if foodLocation:
        minFoodDist = min([util.manhattanDistance(position, food) for food in foodLocation])
    minFoodDist = 10.0 / minFoodDist if minFoodDist else minFoodDist
    features['nearestFood'] = minFoodDist

    ghostLocation = state.getGhostPositions()
    nearestGhost = 0
    if ghostLocation:
        nearestGhost = min(util.manhattanDistance(loc, position) for loc in ghostLocation)
    if nearestGhost:
        nearestGhost = 10.0 / nearestGhost
    features['nearestGhostDist'] = nearestGhost
    #util.raiseNotDefined()
    return features
