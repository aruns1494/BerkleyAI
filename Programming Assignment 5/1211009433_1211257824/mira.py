            ...
            for n in range(self.max_iterations):
                for i, data in enumerate(trainingData):
                    ...
                    if maxY != actualY:
                        ...
                        val = data.copy()
                        minimum = min(grid, ((weights[maxY] - weights[actualY]) * val + 1.0) / (2.0 * (val * val)))
                        val.divideAll(1.0 / minimum)

                        weights[actualY] = weights[actualY] + val
                        weights[maxY] = weights[maxY] - val
            correct = 0
