...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:                       # To check if the Pacman is in the same cell as ghost
            allPossible = util.Counter()
            allPossible[self.getJailPosition()] = 1.0
        else:
            for positions in self.legalPositions:
                manhattanDistance = util.manhattanDistance(positions, pacmanPosition) # Obtain the manhattan distance
                allPossible[positions] = emissionModel[manhattanDistance] * self.beliefs[positions]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            ...
            for newPosition, probabilityValue in newPositionDistribution.items():
                allPossible[newPosition] = allPossible[newPosition] + probabilityValue*self.beliefs[oldPosition]
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        currentParticleNumber = 0   # Current Particle Number is a counter variable
        self.particles = []
        while currentParticleNumber < self.numParticles:
            for currentPosition in self.legalPositions:
                if currentParticleNumber < self.numParticles:
                    self.particles.append(currentPosition)
                    currentParticleNumber = currentParticleNumber + 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:  # Case 1 being Ghost is captured by the Pacman
            self.particles = [self.getJailPosition()] * self.numParticles
        else:
            allPossible = util.Counter()
            oldBelief =  self.getBeliefDistribution()   # Old belief value
            for currentPosition in self.legalPositions:
                distance = util.manhattanDistance(currentPosition, pacmanPosition)  # manhattan distance
                allPossible[currentPosition] = allPossible[currentPosition] + (emissionModel[distance] * oldBelief[currentPosition])    # update weights
            if not any(allPossible.values()):  # Case 2 being all particles receive a zero weight
                self.initializeUniformly(gameState) # initializeUniformly being called to recreate from Prior Distribution
            else:       # In case of non-zero weights
                temp = []
                for i in range(0, self.numParticles):
                    temp.append(util.sample(allPossible))  # recreate samples based on distribution allPossible
                self.particles = temp                       # Update self.particles
        # util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        temporaryParticles = []
        for oldParticlePosition in self.particles:      # Start Loop
            newPositionDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, oldParticlePosition))   # Compute the new ditribution
            temporaryParticles.append(util.sample(newPositionDistribution))     # Append Position Distribution
    ...
    def getBeliefDistribution(self):
        ...
        for curParticle in self.particles:
            distribution[curParticle] = distribution[curParticle] + 1
        distribution.normalize() # Normlize the distribution
        return distribution
        # util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        possiblePosition = list(itertools.product(self.legalPositions, repeat = self.numGhosts)) # performs Cartesian product in self.legalPositions
        random.shuffle(possiblePosition)    # Randomizing the possiblePosition variable
        particleNumber = 0
        self.particles = []
        while particleNumber < self.numParticles:
            for position in possiblePosition:
                if particleNumber < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        for index, particle in enumerate(self.particles):
            temp = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:  # Case 1 where Pacman and Ghost are in the same cell leading to the ghost being jailed
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    distance = util.manhattanDistance(particle[i], pacmanPosition)  # Calculate Manhattan Distance
                    temp = temp * emissionModels[i][distance]
            allPossible[particle] = allPossible[particle] + temp

        if not any(allPossible.values()):   # Case 2 where all weights are 0
            self.initializeParticles()      # Particles recreated
            for index, particle in enumerate(self.particles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:       # Check if PACMAN and Ghost are in the same cell
                        particle = self.getParticleWithGhostInJail(particle, i)     # Place it in jail
        else:
            allPossible.normalize()     # Normalize
            temp = []
            for _ in range(0, self.numParticles):
                temp.append(util.sample(allPossible))   # Append sample particles
            self.particles = temp   # Reform particles
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            allPossible[particle] = allPossible[particle] + 1.0
        allPossible.normalize()
        return allPossible
